...
class Or_AST:
    def __init__(self, c1, c2):
        ...
        self.c2 = c2
    def indented(self, level):
        return indent("Or", level) + self.c1.indented(level + 1)\
                        + self.c2.indented(level + 1)
    def __repr__(self):
        return repr(self.c1) + " or " + repr(self.c2)
    
    def true_code(self, label):
        # jump if true
        return self.c1.true_code(label) + self.c2.true_code(label)

    def false_code(self, label):
        # jump if false
        t1 = label_generator.next()
        return self.c1.true_code(t1) + self.c2.false_code(label) + t1 + ":\n"
...
class And_AST:
    ...
    def __repr__(self, c1, c2):
        return repr(self.c1) + " and " + repr(self.c2)
    
    def indented(self, level):
        return indent("And", level) + self.c1.indented(level + 1)\
                        + self.c2.indented(level + 1)

    def true_code(self, label):
        # jumps if both exprs are true
        t1 = label_generator.next()
        return self.c1.false_code(t1) + self.c2.true_code(label) + t1 + ':\n'

    def false_code(self, label):
        # jumps if either expr is false
        return self.c1.false_code(label) + self.c2.false_code(label)
...
class Not_AST:
    ...
    def __repr__(self):
        return "not " + repr(self.exp)

    def indented(self, level):
        return indent("Not", level) + self.exp.indented(level + 1) 

    def false_code(self, label):
        return self.exp.true_code(label)

    def true_code(self, label):
        return self.exp.false_code(label)
...
while n <= 60 do
    ...
    if n = 5 or n = 10 or not n <= 30 then
        out := out * 5
    else
        out := out * 7
    end;
...
if n > 2 and n < 5 or not i < 2 then
    write n
end
...
if z > 0 and z < 5 and z = 1 then
    write z
end;

if z > 0 or z < 5 or z = 1 then
    write z
end;
...
import os
print("\nAST Compiled  DONE\n")
os.system("java -Xmx100m -jar jasmin.jar Program.j")
print("\nJava bytecode DONE\n")
os.system("java Program")
#'''
